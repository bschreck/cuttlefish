...
class LV2TransportController(FaderfoxTransportController):
    ...
    __filter_funcs__ = ['update_display',
     'log']
    ...
    def set_slot_launch_led(self, track_idx, clip_idx, playing):
        self.log(('set slot launch led %s %s %s' % (track_idx,
         clip_idx,
         playing)))
        ...
        if playing:
            self.parent.send_midi(((NOTEON_STATUS + channel),
             note_no,
             127))
        else:
            self.parent.send_midi(((NOTEOFF_STATUS + channel),
             note_no,
             0))
    ...
    def update_track_playing_status(self):
        ...
        for track in self.parent.song().tracks:
            ...
            if self.helper.is_track_playing(track):
                self.parent.send_midi(((NOTEON_STATUS + TRACK_CHANNEL_SETUP2),
                 LAUNCH_NOTES[i],
                 127))
            else:
                self.parent.send_midi(((NOTEOFF_STATUS + TRACK_CHANNEL_SETUP2),
                 LAUNCH_NOTES[i],
                 0))
            i += 1
