        ...
        if (((midi_bytes[0] & 240) == NOTE_ON_STATUS) or ((midi_bytes[0] & 240) == NOTE_OFF_STATUS)):
            ...
            if (note in range(SID_FIRST, (SID_LAST + 1))):
                ...
                if (note in channel_strip_control_switch_ids):
                    self._ProjectMixIO__channel_strip_controller.handle_assignment_switch_ids(note, velocity)
                if (note in function_key_control_switch_ids):
                    self._ProjectMixIO__software_controller.handle_function_key_switch_ids(note, velocity)
                if (note in software_controls_switch_ids):
                    self._ProjectMixIO__software_controller.handle_software_controls_switch_ids(note, velocity)
                if (note in transport_control_switch_ids):
                    self._ProjectMixIO__transport.handle_transport_switch_ids(note, velocity)
                if (note in marker_control_switch_ids):
                    self._ProjectMixIO__transport.handle_marker_switch_ids(note, velocity)
                if (note in jog_wheel_switch_ids):
                    self._ProjectMixIO__transport.handle_jog_wheel_switch_ids(note, velocity)
